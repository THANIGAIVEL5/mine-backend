...
class TelemetryClient {
    ...
    toggle3DContour() {
        ...
        if (btn) {
            if (this.is3DActive) {
                ...
                if (container)
                    container.style.transform = 'perspective(900px) rotateX(12deg)';
            }
            else {
    ...
    renderDgmsProtocolLayers() {
        if (!this.protocolLayerGroup)
            return;
    ...
    setMapMode(mode) {
        ...
        if (mode === 'satellite') {
            if (btnSat)
                btnSat.className = "bg-white text-black font-bold font-['JetBrains_Mono'] text-[10px] uppercase px-2.5 py-1 rounded transition shadow-[0_0_10px_rgba(255,255,255,0.4)] cursor-pointer";
            if (realMapEl) {
            ...
            if (this.leafletMap) {
                if (this.darkLayer && this.leafletMap.hasLayer(this.darkLayer))
                    this.leafletMap.removeLayer(this.darkLayer);
                if (this.satelliteLayer && !this.leafletMap.hasLayer(this.satelliteLayer))
                    this.satelliteLayer.addTo(this.leafletMap);
                this.leafletMap.invalidateSize();
            }
        }
            ...
            if (btnDark)
                btnDark.className = "bg-white text-black font-bold font-['JetBrains_Mono'] text-[10px] uppercase px-2.5 py-1 rounded transition shadow-[0_0_10px_rgba(255,255,255,0.4)] cursor-pointer";
            if (realMapEl) {
                realMapEl.style.display = 'block';
                realMapEl.style.opacity = '1';
                realMapEl.style.pointerEvents = 'auto';
            ...
            if (this.leafletMap) {
                ...
                if (this.darkLayer && !this.leafletMap.hasLayer(this.darkLayer))
                    this.darkLayer.addTo(this.leafletMap);
                this.leafletMap.invalidateSize();
            }
        }
        else if (mode === 'schematic') {
            if (btnSchem)
                btnSchem.className = "bg-white text-black font-bold font-['JetBrains_Mono'] text-[10px] uppercase px-2.5 py-1 rounded transition shadow-[0_0_10px_rgba(255,255,255,0.4)] cursor-pointer";
            if (realMapEl) {
    ...
    toggleProtocolLayer() {
        if (!this.leafletMap || !this.protocolLayerGroup)
            return;
