    ...
    .controller('CommandPaletteController', [
      ...
      function ($scope, $document, $timeout, audioService, themeService) {
        ...
        var ALL_COMMANDS = [
          ...
          { icon: 'database',        label: 'SQL: Show Incident Alerts in Sector 4B', category: 'TEXT-TO-SQL', action: function () { var btn = document.getElementById('chat-toggle-btn'); if (btn) btn.click(); var scope = angular.element(document.querySelector('[ng-controller*="ChatWidgetController"]')).scope(); if (scope && scope.chat) { scope.chat.activeMode = 'SQL'; scope.chat.messageInput = 'Show all incident alerts in Sector 4B'; scope.chat.sendMessage(); } } },
          { icon: 'database',        label: 'SQL: List Active Underground Miners',    category: 'TEXT-TO-SQL', action: function () { var btn = document.getElementById('chat-toggle-btn'); if (btn) btn.click(); var scope = angular.element(document.querySelector('[ng-controller*="ChatWidgetController"]')).scope(); if (scope && scope.chat) { scope.chat.activeMode = 'SQL'; scope.chat.messageInput = 'List active underground miners and their SCSR status'; scope.chat.sendMessage(); } } },
          { icon: 'database',        label: 'SQL: Query DGMS Form IV Safety Audits',  category: 'TEXT-TO-SQL', action: function () { var btn = document.getElementById('chat-toggle-btn'); if (btn) btn.click(); var scope = angular.element(document.querySelector('[ng-controller*="ChatWidgetController"]')).scope(); if (scope && scope.chat) { scope.chat.activeMode = 'SQL'; scope.chat.messageInput = 'Show all DGMS Form IV safety audits'; scope.chat.sendMessage(); } } },
          ...
          { icon: 'terminal',        label: 'Open AI Command Terminal',             category: 'AI',       action: function () { $scope.$broadcast('cmd:openTerminal'); if ($scope.$root) { $scope.$root.$broadcast('cmd:openTerminal'); } var btn = document.getElementById('chat-toggle-btn'); if (btn) btn.click(); } },
        ];
